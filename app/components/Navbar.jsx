...
const Navbar = ({ isDarkMode, setIsDarkMode }) => {
    ...
    return (
        <>
            ...
            <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white/50  backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20 dark:border-b dark:border-solid dark:border-white/20" : ""}`}>
                ...
                {/* big screen */}
                <ul className={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? '' : 'bg-white/50 shadow-sm dark:border dark:border-solid dark:border-white/20 dark:bg-transparent'}`}>
                ...
                <div className='flex items-center gap-2'>
                    ...
                    {/* contact button */}
                    <a href="#contact" className='hidden lg:flex items-center gap-3 px-10 py-2.5 border border-solid border-gray-500 rounded-full ml-4 font-Ovo dark:border-white/50 hover:bg-gray-200 dark:hover:bg-darkHover ' style={{ fontFamily: "var(--font-ovo)" }}>
                ...
                {/* mobile menu */}
                <ul ref={sideMenuRef} style={{ backgroundColor: isDarkMode ? '#2a004a' : '#fff1f2' }} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen transition duration-500 dark:text-white'>
